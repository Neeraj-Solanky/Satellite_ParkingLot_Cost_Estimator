...
var panel = ui.Panel({
    widgets: [
        ...
        ui.Label('3. Start drawing the polygon directly on the map.'),
        ui.Label('4. Click "Erase Last" to remove mistakes.'),
        ui.Label('5. Click "Compute Area" to calculate details or press Enter after drawing.')
    ],
...
var minimizeButton = ui.Button({
    label: '🔽 Minimize',
    onClick: function() {
        var isVisible = panel.style().get('shown');
        panel.style().set('shown', !isVisible);
        minimizeButton.setLabel(isVisible ? '🔼 Expand' : '🔽 Minimize');
    }
});

// Create a separate panel to hold the minimize button when collapsed
var togglePanel = ui.Panel({
    widgets: [minimizeButton],
    style: {position: 'top-left', width: '100px', shown: true}
});
